...
class WordGenerator {
    ...
    static let possibleWords = ["HANDS", "LEMON", "PEELS", "STAIR", "LEMON"]
    
    static func generateRandomWord() -> String? {
        // START YOUR CODE HERE
        // Use the randomElement() method to get a random word from the possibleWords array
        if let randomWord = possibleWords.randomElement() {
            print("goal word: \(randomWord)")
            return randomWord
        } else {
            print("no goal word generated.")
            return nil
        }
        // END YOUR CODE HERE
    }
